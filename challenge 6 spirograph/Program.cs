...
namespace challenge_6_spirograph
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Planet Jupiter = new Planet("Jupiter", 317.85, 5.20, 11.21, 0.41, 11.86, 0.05, 1.31, System.Drawing.Color.Yellow);
            ...
            Planet planet1 = Venus;
            Planet planet2 = Mercury;

            int numsteps = 1233; // No common factors with number of periods
            int numorbits = 10;
            double maxtime = numorbits * (Math.Max(planet1.orbperiod, planet2.orbperiod));
            ...
            while (time <= maxtime)
            {
                double theta1 = 2 * Math.PI * time / planet1.orbperiod;
                double r1 = (planet1.smaxis * (1 - Math.Pow(planet1.ecc, 2))) / (1 - (planet1.ecc * Math.Cos(theta1))); // Polar equation of an ellipse r= a(1-e^2)/(1-e cos(t))
                double x1 = r1 * Math.Sin(theta1);
                double y1 = r1 * Math.Cos(theta1);

                double theta2 = 2 * Math.PI * time / planet2.orbperiod;
                double r2 = (planet2.smaxis * (1 - Math.Pow(planet2.ecc, 2))) / (1 - (planet2.ecc * Math.Cos(theta2))); // Polar equation of an ellipse r= a(1-e^2)/(1-e cos(t))
                double x2 = r2 * Math.Sin(theta2);
                double y2 = r2 * Math.Cos(theta2);

                plt.AddLine(x1, y1, x2, y2, color:System.Drawing.Color.Black);
                time += timestep;
            }
            plt.Title(planet1.name + "-" + planet2.name + " spirograph");
            plt.SaveFig(planet1.name + planet2.name + ".png");
        }
    }
}
